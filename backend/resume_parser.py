...
def _extract_text_from_pdf(p: Path) -> str:
    if pdfplumber is None:
        print("[WARN] pdfplumber not installed. Cannot parse PDF.")
        return ""
    text = []
...
def _extract_text_from_docx(p: Path) -> str:
    if docx is None:
        print("[WARN] python-docx not installed. Cannot parse DOCX.")
        return ""
...
def _generate_ai_summary(parsed_data: Dict) -> str:
    """
    Generates a professional summary for the resume using the Gemini API.
    Dynamically finds a compatible model to ensure reliability.
    """
    if genai is None:
    ...
    try:
        ...
        if not available_models:
            return "[Error: Could not find any compatible models for your API key. Please check your Google Cloud project.]"

        # Use the first compatible model found (e.g., 'models/gemini-pro')
        model_name_to_use = available_models[0]
        print(f"[INFO] Using Gemini model: {model_name_to_use}")
...
def parse_resume(file_path: str) -> Dict:
    ...
    try:
        ...
        print(f"[ERROR] Error during text extraction: {e}")
        return {"error": str(e)}
    ...
    try:
        ...
        print(f"[SUCCESS] Successfully saved parsed resume to: {output_path}")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    print(f"Parsed resume: {result.get('name', 'Unknown')}")